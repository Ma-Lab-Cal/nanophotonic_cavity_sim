...
hand-tuned baseline.
(b) Decomposition of the proxy objective $\mathcal L$ [Eq.~\eqref{eq:loss}]: the
interface term $\mathcal L_\mathrm{int}=-\log\tilde\eta$ falls smoothly, while the
large transients in $\mathcal L$ are carried almost entirely by the resonance
penalty $\mathcal L_\mathrm{freq}$ when the frequency temporarily drifts off
target.
...
refines the mode. Decomposing the proxy objective [Fig.~\ref{fig:opt}(b)] shows
that its interface term $-\log\tilde\eta$ falls smoothly, while the large
transients are carried by the resonance penalty when the frequency temporarily
drifts off target---an honest signature of the multi-objective landscape, not a
failure of the interface optimization. The structure snapshots
