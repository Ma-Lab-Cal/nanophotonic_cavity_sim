...
dashed line marks the $250\unit{nm}$ atom height, where the evanescent field
couples to the trapped atom.
(b) TE Bloch band structure of the mirror and defect unit cells: the mirror band
gap (shading) brackets the Rb~D2 line (dashed), confining the mode, while the
defect band edge sits just below it (light line shown for reference).
(c) $|\mathbf E|$ in the beam midplane ($z=0$) and (d) in the longitudinal plane
($y=0$, atom height dashed); the mode is tightly localized at the defect.
(e) The real part $\mathrm{Re}\,E_y$ in the longitudinal plane, showing the
standing-wave structure of the confined mode. Panels (c)--(e) share the
true-aspect $x$ axis.}
\end{figure*}
